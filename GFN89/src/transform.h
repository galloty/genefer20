...
// 			for (; lm > 4; s *= 16, lm -= 4) { _engine.forward16x_P12(s, lm - 4); _engine.forward16x_P3(s, lm - 4); }
// 			if (lm > 2) _engine.forward4x_P123(s, lm - 2);
// 			if (lm % 2 != 0) _engine.mul2cond1024xy_P123(c); else _engine.mul4cond1024xy_P123(c);
// 			if (lm > 2) _engine.backward4x_P123(s, lm - 2);
// 			for (s /= 16, lm += 4; s > 0; s /= 16, lm += 4) { _engine.backward16x_P12(s, lm - 4); _engine.backward16x_P3(s, lm - 4); }
// 			_engine.normalize3ax();
// 			_engine.normalize3bx();
...
// 			for (; lm > 4; s *= 16, lm -= 4) _engine.forward16x_P12(s, lm - 4);
// 			if (lm > 2) _engine.forward4x_P12(s, lm - 2);
// 			if (lm % 2 != 0) _engine.mul2cond1024xy_P12(c); else _engine.mul4cond1024xy_P12(c);
// 			if (lm > 2) _engine.backward4x_P12(s, lm - 2);
// 			for (s /= 16, lm += 4; s > 0; s /= 16, lm += 4) _engine.backward16x_P12(s, lm - 4);
// 			_engine.normalize2ax();
// 			_engine.normalize2bx();
...
private:
	void mulMod1024_4(const uint64_16 & c) const
	{
		const size_t n = this->_n;
		const int ln = ilog2(uint32_t(n));
...
public:
		...
		_x(new uint32[VSIZE_MAX * size]), _gx(new uint32_2[VSIZE_MAX * size]), _gd(new uint32_2[VSIZE_MAX * size])
	{
		if (vsize == 0)
		{
			...
			{
				this->_csize = csize;
				for (size_t vsize = 64; vsize <= VSIZE_MAX; vsize *= 2)
				{
					this->_vsize = vsize;
					...
					std::cout << "radix-4, csize = " << csize << ", vsize = " << vsize
		...
		std::cout << "Radix = 4, vector size = " << this->_vsize << ", chunk size = " << this->_csize << "         " << std::endl << std::endl;
...
public:
	...
	bool getRadix16() const { return false; }
...
public:
	void gerbiczStep() const
	{
		...
		{
			_engine.setxy_P123();
			for (size_t s = 1, lm = ln; lm > 2; s *= 4, lm -= 2) _engine.forward4y_P123(s, lm - 2);
			size_t s = 1, lm = ln;
			for (; lm > 2; s *= 4, lm -= 2) _engine.forward4d_P123(s, lm - 2);
			if (lm % 2 != 0) _engine.mul2dy_P123(); else _engine.mul4dy_P123();
			for (s /= 4, lm += 2; s > 0; s /= 4, lm += 2) _engine.backward4d_P123(s, lm - 2);
		...
		{
			_engine.setxy_P12();
			for (size_t s = 1, lm = ln; lm > 2; s *= 4, lm -= 2) _engine.forward4y_P12(s, lm - 2);
			size_t s = 1, lm = ln;
			for (; lm > 2; s *= 4, lm -= 2) _engine.forward4d_P12(s, lm - 2);
			if (lm % 2 != 0) _engine.mul2dy_P12(); else _engine.mul4dy_P12();
			for (s /= 4, lm += 2; s > 0; s /= 4, lm += 2) _engine.backward4d_P12(s, lm - 2);
